...
public class NPCNetHandler extends NetServerHandler {
	...
	public CraftPlayer getPlayer() {
		NPCCreatures plugin = ((NPCCreatures) Bukkit.getServer().getPluginManager().getPlugin("NPCCreatures"));
		if(plugin.isSpoutEnabled) {
			try {
				return (CraftPlayer) Class.forName("org.getspout.spout.player.SpoutCraftPlayer").getConstructor(CraftServer.class, EntityPlayer.class).newInstance((CraftServer) Bukkit.getServer(), player);
			} catch(Exception e) {
				NPCCreatures.logger.warning("Failed to create SpoutCraftPlayer instance in NPCNetHandler, returning CraftPlayer.");
